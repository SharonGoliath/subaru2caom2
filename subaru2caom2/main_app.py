...
from caom2 import DataProductType, CalibrationLevel, ProductType, caom_util, Part
from caom2utils import data_util, ObsBlueprint, update_artifact_meta
...
class SubaruName(mc.StorageName):
    ...
    def __init__(self, entry):
        temp = entry.replace('.header', '')
        super().__init__(file_name=basename(temp), source_names=[entry])

    def set_obs_id(self, **kwargs):
        if self.is_legacy:
            bits = self._file_name.split('.')
            self._obs_id = '.'.join(kk for kk in bits[:3])
        else:
            self._obs_id = self._file_name[:11]

    def set_product_id(self):
        self._product_id = SubaruName.remove_extensions(self._file_name)
...
class Telescope(cc.TelescopeMapping):
    ...
    def accumulate_blueprint(self, bp):
        ...
        bp.add_attribute('Observation.type', 'DATA-TYP')

        bp.set('Observation.environment.photometric', True)
        bp.add_attribute('Observation.environment.seeing', 'IQFINAL')

        bp.set('Observation.instrument.name', 'Suprime-Cam')
        if '.cat' in self._storage_name.file_uri:
        ...
        if self._storage_name.is_legacy:
        ...
        bp.add_attribute('Plane.metrics.magLimit', 'ML_5SIGA')
        ...
        if self._storage_name.is_legacy:
            ...
            # bp.add_attribute('Plane.provenance.reference', 'SOFTINST')
            # SGw 7-09-21
        ...
        elif '.cat' in self._storage_name.file_uri:
    ...
    def update(self, file_info):
        ...
        min_seeing = None
        if (
            self._observation.environment is not None
            and self._observation.environment.seeing is not None
        ):
            min_seeing = self._observation.environment.seeing
        for plane in self._observation.planes.values():
            if (
                '.weight' not in self._storage_name.file_name
                and '.cat' not in self._storage_name.file_name
                and self._storage_name.product_id == plane.product_id
            ):
                if self._storage_name.is_legacy and plane.provenance is not None:
            ...
            for artifact in plane.artifacts.values():
                ...
                self._update_weight_artifact(plane)

        if self._observation.environment is not None:
            self._observation.environment.seeing = min_seeing
        if self._storage_name.is_legacy:
    ...
    def _get_time_function_val(self, ext):
        ...
        if date_obs is not None and ut_str is not None:
            ...
            start_time = ac.get_datetime_mjd(temp).value
        return start_time
    ...
    def _update_observation_metadata(self):
        ...
        """
        self._logger.debug(
            f'Begin _update_observation_metadata for {self._observation.observation_id}'
        )
        # headers is not None => .cat files
        if (
            self._headers is not None
            and len(self._headers) > 1
            and not self._storage_name.is_legacy
        ):
            from urllib.parse import urlparse
            bits = urlparse(self._storage_name.source_names[0])
            if bits.scheme == '':
    ...
    def _update_weight_artifact(self, plane):
        """Use the same WCS for the weight artifact as for the 'p' artifact.
        Because cutouts.
        """
        self._logger.debug(
            f'Begin _update_weight_artifact for {self._observation.observation_id}'
        )
        if self._storage_name.is_legacy:
            # only applicable to 'p' files
            return

        if 'weight' in self._storage_name.file_name:
            w_artifact_key = self._storage_name.file_uri
            p_artifact_key = self._storage_name.file_uri.replace('.weight', '')
        else:
            p_artifact_key = self._storage_name.file_uri
            w_artifact_key = self._storage_name.file_uri.replace(
                '.fits', '.weight.fits'
            )
        ...
        if w_artifact_key in plane.artifacts.keys():
            ...
            if p_artifact_key in plane.artifacts.keys():
                ...
                for part in p_artifact.parts.values():
                    ...
                    for chunk in part.chunks:
                        w_artifact.parts[part.name].chunks.append(cc.copy_chunk(chunk))
